...
import { Announcement, AnnouncementWithAuthor } from "@/types";

interface BroadcastBoxProps {
  broadcasts: AnnouncementWithAuthor[];
...
const typeStyles: Record<string, string> = {
  ...
  warning:
    "bg-yellow-500/10 border-yellow-500/30 text-yellow-900 dark:text-yellow-100",
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 text-xs opacity-75">
                ...
                {broadcast.author && (
                  <>
                    <span>•</span>
                    <span>by {broadcast.author.name}</span>
                  </>
                )}
              </div>
            </div>
